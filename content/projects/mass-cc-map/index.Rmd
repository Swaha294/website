...
mass_cc_enrollment <- mass_county_census %>% 
  mutate(
...
mass_cc_total <- mass_cc_race %>% 
  ...
  summarise(
    total_fall_enrollment = sum(count)
  )

mass_cc_enrollment <- mass_cc_total %>% 
  ...
  mutate(
    total_fall_enrollment = ifelse(is.na(total_fall_enrollment), 0, total_fall_enrollment)
  ) %>% 
  left_join(
    mass_income_county,
    by = c("county" = "county")
...
mass_cc_st <- mass_geo_county %>% 
  left_join(mass_cc_enrollment, by = c("County" = "county")) %>% 
  ungroup()
